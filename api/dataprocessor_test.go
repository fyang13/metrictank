...
// has been used correctly. (may want to switch to the new StorageStats at some point)
//
// cache:                             |-----|-----|
// store:           |-----|-----|-----|-----|
//
			...
			for to := from; to <= lastTs; to += step {
				...
				for i := 0; i < len(tc.Pattern); i++ {
					...
					if pattern[i] == 's' || pattern[i] == 'b' {
						cwr := mdata.NewChunkWriteRequest(nil, metric, 0, chunks[i].Series.T0, encodedChunk, time.Now())
						store.Add(&cwr)
					}
